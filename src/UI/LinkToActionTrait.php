...
use WebChemistry\Link\StringActionLink;
...
trait LinkToActionTrait
{
	 ...
	 * @return never
	 */
	public function redirectToAction(string|object $destination, mixed ... $arguments): void
	{
		$link = $this->_linkGenerator->createActionLink($destination, $this->parseArguments($arguments));

		if ($link instanceof StringActionLink) {
			$this->redirect($destination, $link->getParameters());
		}
	...
	public function linkToAction(string|object $destination, mixed ... $arguments): string
	{
		$link = $this->_linkGenerator->createActionLink($destination, $this->parseArguments($arguments));
		$destination = $link->getDestination();

		if ($link instanceof StringActionLink) {
			return $this->link($destination, $link->getParameters());
		}
